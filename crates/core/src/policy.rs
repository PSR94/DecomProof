...
#[cfg(test)]mod tests{use super::*;use crate::{target::Target,evidence::Evidence};#[test]fn missing_runtime_fails_conservatively(){let r=evaluate(&PolicyConfig::default(),&[],Utc::now());assert!(r.decisions.iter().any(|d|!d.passed&&d.effect=="insufficient-evidence"));}#[test]fn documentation_is_soft_not_infra(){let t:Target="service:x".parse().unwrap();let mut e=Evidence::build("documentation.reference","scanner",&t,Utc::now(),None,b"x");e.normalized.insert("active".into(),true.into());let r=evaluate(&PolicyConfig::default(),&[e],Utc::now());assert!(r.blockers.iter().any(|b|!b.hard&&b.kind=="documentation-reference"));assert!(!r.blockers.iter().any(|b|b.kind=="infrastructure-present"));}}
