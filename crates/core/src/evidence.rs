...
pub enum Confidence { Low, Medium, High, Confirmed }

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum ConsumerIdentity { Identified, Partial, Unknown }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ObservationWindow { pub start: DateTime<Utc>, pub end: DateTime<Utc>, #[serde(default)] pub gaps_seconds: u64 }
impl ObservationWindow { pub fn seconds(&self) -> i64 { (self.end - self.start).num_seconds().max(0) } }
...
impl Evidence {
    pub fn build(signal: &str, source: &str, target: &Target, observed_at: DateTime<Utc>, artifact: Option<String>, raw: &[u8]) -> Self {
...
pub struct Uncertainty {
    ...
    pub message: String,
    pub severity: String,
    #[serde(default)] pub evidence_ids: Vec<String>,
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn evidence_id_is_content_stable() { let t: Target = "service:x".parse().unwrap(); let now = Utc::now(); let a = Evidence::build("static.reference","scanner",&t,now,None,b"same"); let b = Evidence::build("static.reference","scanner",&t,now,None,b"same"); assert_eq!(a.id,b.id); }
}
